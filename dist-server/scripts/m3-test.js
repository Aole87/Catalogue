...
async function runM3Tests() {
    ...
    const loginUser = async (email, password = 'Admin@123456') => {
    ...
    try {
        ...
        await test('GET /api/v1/categories/tree returns hierarchical nested category tree', async () => {
            ...
            if (!Array.isArray(body.data) || body.data.length === 0)
                throw new Error('Category tree empty');
            // Verify nested children structure
            const brakes = body.data.find((c) => c.slug === 'brakes');
        ...
        await test('POST /api/v1/admin/categories creates root and child categories (CATALOG_MANAGER)', async () => {
            ...
            if (res1.statusCode !== 201)
                throw new Error(`Failed to create root category: ${res1.payload}`);
            ...
            if (res2.statusCode !== 201)
                throw new Error(`Failed to create child category: ${res2.payload}`);
            const body2 = JSON.parse(res2.payload);
            testChildCategoryId = body2.data.id;
            if (body2.data.parentId !== testParentCategoryId)
        ...
        await test('Reject duplicate category slug with 409 Conflict', async () => {
            ...
            if (res.statusCode !== 409)
                throw new Error(`Expected 409, got ${res.statusCode}`);
        });
        await test('Reject circular hierarchy assignment with 400 Bad Request', async () => {
            ...
            if (res.statusCode !== 400)
                throw new Error(`Expected 400 Circular Error, got ${res.statusCode}`);
        });
        ...
        await test('GET /api/v1/brands returns public brand list', async () => {
            ...
            const body = JSON.parse(res.payload);
            if (!Array.isArray(body.data) || body.data.length === 0)
                throw new Error('Brand list empty');
        });
        ...
        const attr = await database_1.prisma.productAttribute.findFirst();
        await test('POST /api/v1/admin/products creates product atomically with prices, images, attributes, cross-refs', async () => {
            const res = await app.inject({
                ...
                payload: {
                    ...
                    weightGrams: 1200,
                    prices: [
                        { tier: database_1.PriceTier.GENERAL, price: 1500.0, compareAtPrice: 1800.0 },
                        { tier: database_1.PriceTier.GARAGE, price: 1200.0 },
                        { tier: database_1.PriceTier.SHOP, price: 1100.0 },
                    ],
                    images: [
                    ...
                    crossReferences: [
                        { referenceType: database_1.ProductReferenceType.OEM, referenceNumber: '45022-TEST-001' },
                    ],
                },
            });
            if (res.statusCode !== 201) {
            ...
            if (body.data.sku !== testProductSku)
                throw new Error('SKU mismatch');
            if (!body.data.prices || body.data.prices.length !== 3)
                throw new Error('Prices not created');
            if (!body.data.images || body.data.images.length !== 2)
        ...
        await test('PATCH /api/v1/admin/products/:id updates product details and attributes', async () => {
            ...
            if (res.statusCode !== 200)
                throw new Error(`Expected 200, got ${res.statusCode}`);
            const body = JSON.parse(res.payload);
            if (!body.data.name.includes('(Updated)'))
        ...
        await test('PUT /api/v1/admin/products/:id/prices updates price tiers with Decimal precision (Gate C)', async () => {
            const res = await app.inject({
                ...
                payload: {
                    prices: [
                        { tier: database_1.PriceTier.GENERAL, price: 1599.5 },
                        { tier: database_1.PriceTier.GARAGE, price: 1250.0 },
                        { tier: database_1.PriceTier.SHOP, price: 1150.0 },
                    ],
                },
            });
            if (res.statusCode !== 200)
                throw new Error(`Expected 200, got ${res.statusCode}`);
            const body = JSON.parse(res.payload);
            const generalPrice = body.data.prices.find((p) => p.tier === 'GENERAL');
            if (generalPrice.price !== '1599.50')
                throw new Error(`Expected Decimal '1599.50', got ${generalPrice.price}`);
        });
        // --------------------------------------------------------------------------
        ...
        await test('GET /api/v1/products returns paginated storefront products with metadata', async () => {
            ...
            if (!body.data || !body.pagination)
                throw new Error('Missing data or pagination');
            if (body.pagination.page !== 1 || body.pagination.pageSize !== 5)
                throw new Error('Pagination params mismatch');
            if (body.data.length > 5)
                throw new Error('Returned more items than pageSize');
        });
        await test('Page size capping enforces maximum of 100 items per page', async () => {
            ...
            if (res.statusCode !== 200)
                throw new Error(`Expected 200, got ${res.statusCode}`);
            const body = JSON.parse(res.payload);
            if (body.pagination.pageSize > 100)
                throw new Error(`Page size was not capped: ${body.pagination.pageSize}`);
        });
        await test('Category filtering includes products in subcategories', async () => {
            ...
            const parentCat = await database_1.prisma.category.findUnique({ where: { id: testParentCategoryId } });
            const res = await app.inject({
                method: 'GET',
            ...
            if (res.statusCode !== 200)
                throw new Error(`Expected 200, got ${res.statusCode}`);
            const body = JSON.parse(res.payload);
            const found = body.data.find((p) => p.id === createdProductId);
            if (!found)
                throw new Error('Product in subcategory was not included in parent category filter');
        });
        await test('Keyword search finds products across name, SKU, and OEM reference number', async () => {
            ...
            if (res.statusCode !== 200)
                throw new Error(`Expected 200, got ${res.statusCode}`);
            const body = JSON.parse(res.payload);
            if (!body.data.find((p) => p.id === createdProductId)) {
        ...
        await test('Whitelist sorting works for price and name (asc/desc)', async () => {
            ...
            if (resAsc.statusCode !== 200)
                throw new Error(`Expected 200, got ${resAsc.statusCode}`);
            const resDesc = await app.inject({ method: 'GET', url: '/api/v1/products?sortBy=name&sortOrder=desc' });
            if (resDesc.statusCode !== 200)
                throw new Error(`Expected 200, got ${resDesc.statusCode}`);
        });
        ...
        await test('Audit trail recorded all product, category, and brand mutations', async () => {
            ...
            if (logs.length < 5)
                throw new Error(`Expected at least 5 audit records, found ${logs.length}`);
        });
        // Clean up test data
        await database_1.prisma.product.deleteMany({ where: { id: createdProductId } }).catch(() => { });
        await database_1.prisma.category.deleteMany({ where: { id: { in: [testChildCategoryId, testParentCategoryId] } } }).catch(() => { });
        await database_1.prisma.brand.deleteMany({ where: { id: testBrandId } }).catch(() => { });
    }
    finally {
        await app.close();
    }
...
//# sourceMappingURL=m3-test.js.map