...
async function runM11TestSuite() {
    ...
    const superAdminUser = await database_1.prisma.user.findFirst({ where: { email: 'admin@mobex.co.th' } });
    ...
    const customerUser = await database_1.prisma.user.findFirst({ where: { email: 'somchai@autoworkshop.com' } });
    ...
    // ----------------------------------------------------------------------------
    let testWarehouse = await database_1.prisma.warehouse.findFirst({
        where: { code: 'WH-MAIN', deletedAt: null },
    });
    if (!testWarehouse) {
        testWarehouse = await database_1.prisma.warehouse.create({
            data: {
    ...
    let testLocation = await database_1.prisma.warehouseLocation.findFirst({
        where: { warehouseId: testWarehouse.id, code: 'LOC-A1-01', deletedAt: null },
    });
    if (!testLocation) {
        testLocation = await database_1.prisma.warehouseLocation.create({
            data: {
    ...
    const sampleCategory = await database_1.prisma.category.findFirst();
    const sampleBrand = await database_1.prisma.brand.findFirst();
    let productA = await database_1.prisma.product.findFirst({
        where: { sku: 'SKU-M11-BRAKE-01', deletedAt: null },
    });
    if (!productA) {
        productA = await database_1.prisma.product.create({
            data: {
                ...
                prices: {
                    create: {
                        tier: 'GENERAL',
                        price: new database_1.Prisma.Decimal('2500.00'),
                        currency: 'THB',
                        isActive: true,
                    },
                },
        ...
        const existingPrice = await database_1.prisma.productPrice.findFirst({ where: { productId: productA.id } });
        if (!existingPrice) {
            await database_1.prisma.productPrice.create({
                data: {
    ...
    let productB = await database_1.prisma.product.findFirst({
        where: { sku: 'SKU-M11-FILTER-02', deletedAt: null },
    });
    if (!productB) {
        productB = await database_1.prisma.product.create({
            data: {
                ...
                prices: {
                    create: {
                        tier: 'GENERAL',
                        price: new database_1.Prisma.Decimal('450.00'),
                        currency: 'THB',
                        isActive: true,
                    },
                },
        ...
        const existingPrice = await database_1.prisma.productPrice.findFirst({ where: { productId: productB.id } });
        if (!existingPrice) {
            await database_1.prisma.productPrice.create({
                data: {
    ...
    let sharedSupplierId = '';
    let sharedSupplierCode = '';
    let sharedPoId = '';
    let sharedPoNumber = '';
    // ----------------------------------------------------------------------------
    ...
    await recordTest('M11-T05: Supplier Soft Delete (Deactivate) — Deactivating sets isActive: false', async () => {
        const tempSup = await database_1.prisma.supplier.create({
            data: {
    ...
    await recordTest('M11-T06: Inactive Supplier PO Creation Block — Cannot create PO for inactive supplier', async () => {
        const inactiveSup = await database_1.prisma.supplier.create({
            data: {
        ...
        await database_1.prisma.supplierProduct.create({
            data: {
    ...
    await recordTest('M11-T07: Inactive Supplier Historical Data Access — Historical POs remain readable after supplier deactivation', async () => {
        ...
        const histSup = await database_1.prisma.supplier.create({
            data: {
        ...
        await database_1.prisma.supplier.update({
            where: { id: histSup.id },
    ...
    await recordTest('M11-T08: Supplier Search, Filter & Pagination — List suppliers with query, active filter, limit and offset', async () => {
        ...
        assert(body.data.some((s) => s.id === sharedSupplierId), 'Shared Denso supplier should be in results');
    });
    ...
    await recordTest('M11-T11: SupplierProduct Preferred Flag Update & Concurrency — Atomic single-preferred flag under sequential and concurrent updates', async () => {
        ...
        const sup2 = await database_1.prisma.supplier.create({
            data: {
        ...
        const map1 = await database_1.prisma.supplierProduct.findFirst({
            where: { supplierId: sharedSupplierId, productId: productA.id },
        });
        const map2 = await database_1.prisma.supplierProduct.findFirst({
        ...
        const preferredCount = await database_1.prisma.supplierProduct.count({
    ...
    await recordTest('M11-T14: Inactive Product Mapping Rejection — Cannot map deleted or inactive product', async () => {
        const inactProd = await database_1.prisma.product.create({
            data: {
    ...
    await recordTest('M11-T17: PO Server-Authoritative Totals Calculation — subtotal, taxAmount (7%), shippingCost, totalAmount via Decimal', async () => {
        ...
        const po = await database_1.prisma.purchaseOrder.findUnique({ where: { id: sharedPoId } });
        assert(po != null, 'PO found');
        const subtotal = po.subtotal;
        const tax = po.taxTotal ?? po.taxAmount;
        const shipping = po.shippingCost;
        const total = po.grandTotal ?? po.totalAmount;
        assert(new database_1.Prisma.Decimal(subtotal).equals(18900), `Subtotal should be 18900, got ${subtotal}`);
        assert(new database_1.Prisma.Decimal(tax).equals(1323), `Tax amount should be 1323, got ${tax}`);
        assert(new database_1.Prisma.Decimal(shipping).equals(200), `Shipping cost should be 200, got ${shipping}`);
        assert(new database_1.Prisma.Decimal(total).equals(20423), `Total amount should be 20423, got ${total}`);
    });
    ...
    await recordTest('M11-T18: PO Item Snapshot Immutability — Items preserve name, productSku and supplierSku snapshots', async () => {
        const po = await database_1.prisma.purchaseOrder.findUnique({
            where: { id: sharedPoId },
            include: { items: true },
        });
        assert(po != null && po.items.length === 2, 'PO items found');
        const itemA = po.items.find((i) => i.productId === productA.id);
    ...
    await recordTest('M11-T28: Historical Procurement Immutability — Cannot modify SENT PO and future vendor/catalog updates do not alter historical PO records', async () => {
        ...
        const poBefore = await database_1.prisma.purchaseOrder.findUnique({
            where: { id: sharedPoId },
            include: { items: true },
        });
        const poItemA = poBefore.items.find((i) => i.productId === productA.id);
        ...
        await database_1.prisma.supplierProduct.updateMany({
            where: { supplierId: sharedSupplierId, productId: productA.id },
        ...
        const poAfter = await database_1.prisma.purchaseOrder.findUnique({
            where: { id: sharedPoId },
            include: { items: true },
        });
        const poItemAfter = poAfter.items.find((i) => i.productId === productA.id);
    ...
    await recordTest('M11-T30: Goods Receipt Full Receiving Workflow — 100% receiving transitions PO SENT -> RECEIVED', async () => {
        ...
        const poDetails = await database_1.prisma.purchaseOrder.findUnique({ where: { id: po.id }, include: { items: true } });
        const itemA = poDetails.items.find((i) => i.productId === productA.id);
        const itemB = poDetails.items.find((i) => i.productId === productB.id);
        ...
        const updatedPo = await database_1.prisma.purchaseOrder.findUnique({ where: { id: po.id } });
    ...
    await recordTest('M11-T31: Goods Receipt Server-Generated GRN Number — Verify format GRN-YYYYMMDD-XXXXX', async () => {
        const receipts = await database_1.prisma.goodsReceipt.findMany({ take: 1, orderBy: { createdAt: 'desc' } });
        assert(receipts.length > 0, 'Receipt exists');
        const grn = receipts[0].receiptNumber || receipts[0].grnNumber;
    ...
    await recordTest('M11-T32: Goods Receipt Partial Receiving — Receiving partial quantities transitions PO SENT -> PARTIALLY_RECEIVED', async () => {
        ...
        const poDetails = await database_1.prisma.purchaseOrder.findUnique({ where: { id: sharedPoId }, include: { items: true } });
        const itemA = poDetails.items.find((i) => i.productId === productA.id);
        const itemB = poDetails.items.find((i) => i.productId === productB.id);
        ...
        const updatedPo = await database_1.prisma.purchaseOrder.findUnique({ where: { id: sharedPoId } });
    ...
    await recordTest('M11-T33: Goods Receipt Multiple Partial Receipts — Sequential receipts finish PO -> RECEIVED', async () => {
        ...
        const finalPo = await database_1.prisma.purchaseOrder.findUnique({
            where: { id: sharedPoId },
            include: { items: true },
        });
        assert(finalPo.status === 'RECEIVED', `PO status should now be RECEIVED, got ${finalPo.status}`);
        const finalItemA = finalPo.items.find((i) => i.productId === productA.id);
        assert(finalItemA.receivedQuantity === 10, 'Item A fully received (10/10)');
        const finalItemB = finalPo.items.find((i) => i.productId === productB.id);
    ...
    await recordTest('M11-T35: Goods Receipt Defective / Rejected Items Handling — Rejected items recorded without incrementing on-hand stock', async () => {
        ...
        await app.inject({ method: 'POST', url: `/api/v1/procurement/orders/${po.id}/approve`, headers: { cookie: adminCookie } });
        await app.inject({ method: 'POST', url: `/api/v1/procurement/orders/${po.id}/send`, headers: { cookie: managerCookie } });
        const item = (await database_1.prisma.purchaseOrderItem.findFirst({ where: { purchaseOrderId: po.id } }));
        // Initial on-hand
        const initStock = await database_1.prisma.inventoryItem.findFirst({
        ...
        const afterStock = await database_1.prisma.inventoryItem.findFirst({
            where: { warehouseId: testWarehouse.id, productId: productA.id },
        });
    ...
    await recordTest('M11-T37: Procurement-to-Inventory Stock Mutation Integration — Atomic onHand increment via M10 InventoryService.receiveStock', async () => {
        ...
        await app.inject({ method: 'POST', url: `/api/v1/procurement/orders/${po.id}/send`, headers: { cookie: managerCookie } });
        const item = (await database_1.prisma.purchaseOrderItem.findFirst({ where: { purchaseOrderId: po.id } }));
        const beforeItem = await database_1.prisma.inventoryItem.findFirst({
        ...
        const afterItem = await database_1.prisma.inventoryItem.findFirst({
            where: { warehouseId: testWarehouse.id, productId: productB.id },
        });
    ...
    await recordTest('M11-T39: True Goods Receipt Idempotency — Sequential retry and concurrent duplicate submissions produce exactly 1 receipt and 1 stock mutation', async () => {
        ...
        const stockBefore = (await database_1.prisma.inventoryItem.findFirst({
            where: { warehouseId: testWarehouse.id, productId: productA.id },
        })).onHand;
        const grCountBefore = await database_1.prisma.goodsReceipt.count({ where: { purchaseOrderId: po.id } });
        const movCountBefore = await database_1.prisma.stockMovement.count({ where: { warehouseId: testWarehouse.id, productId: productA.id, movementType: 'PURCHASE_RECEIPT' } });
        // Request 1
        ...
        const stockAfter1 = (await database_1.prisma.inventoryItem.findFirst({ where: { warehouseId: testWarehouse.id, productId: productA.id } })).onHand;
        const grCountAfter1 = await database_1.prisma.goodsReceipt.count({ where: { purchaseOrderId: po.id } });
        const movCountAfter1 = await database_1.prisma.stockMovement.count({ where: { warehouseId: testWarehouse.id, productId: productA.id, movementType: 'PURCHASE_RECEIPT' } });
        ...
        const stockAfter2 = (await database_1.prisma.inventoryItem.findFirst({ where: { warehouseId: testWarehouse.id, productId: productA.id } })).onHand;
        const grCountAfter2 = await database_1.prisma.goodsReceipt.count({ where: { purchaseOrderId: po.id } });
        const movCountAfter2 = await database_1.prisma.stockMovement.count({ where: { warehouseId: testWarehouse.id, productId: productA.id, movementType: 'PURCHASE_RECEIPT' } });
        ...
        const itemCon = (await database_1.prisma.purchaseOrderItem.findFirst({ where: { purchaseOrderId: poCon.id } }));
        const concurrentKey = `IDEMP-CONCURRENT-${Date.now()}`;
        const conStockBefore = (await database_1.prisma.inventoryItem.findFirst({ where: { warehouseId: testWarehouse.id, productId: productA.id } })).onHand;
        ...
        const conRes1 = c1.value;
        const conRes2 = c2.value;
        ...
        const conStockAfter = (await database_1.prisma.inventoryItem.findFirst({ where: { warehouseId: testWarehouse.id, productId: productA.id } })).onHand;
        ...
        const conGrCount = await database_1.prisma.goodsReceipt.count({ where: { purchaseOrderId: poCon.id } });
    ...
    await recordTest('M11-T40: Over-Receiving & Idempotency Interaction — Idempotency retry does not exceed ordered quantity, new key fails with 400', async () => {
        ...
        const poAfter80 = await database_1.prisma.purchaseOrder.findUnique({ where: { id: po.id }, include: { items: true } });
        ...
        const poAfter100 = await database_1.prisma.purchaseOrder.findUnique({ where: { id: po.id }, include: { items: true } });
        ...
        const poAfterRetry = await database_1.prisma.purchaseOrder.findUnique({ where: { id: po.id }, include: { items: true } });
    ...
    await recordTest('M11-T42: Procurement RBAC — Customer access rejected with 403 Forbidden', async () => {
        const routesToTest = [
            ...
            { method: 'POST', url: '/api/v1/suppliers', payload: { code: 'HACK', name: 'Hack Co' } },
            { method: 'GET', url: '/api/v1/procurement/orders' },
            { method: 'POST', url: '/api/v1/procurement/orders', payload: {} },
            { method: 'POST', url: '/api/v1/procurement/receipts', payload: {} },
        ];
    ...
    await recordTest('M11-T45: Atomic Transaction Rollback on Inventory Failure — GoodsReceipt rolls back cleanly if sub-operation fails', async () => {
        ...
        await app.inject({ method: 'POST', url: `/api/v1/procurement/orders/${po.id}/send`, headers: { cookie: managerCookie } });
        const item = (await database_1.prisma.purchaseOrderItem.findFirst({ where: { purchaseOrderId: po.id } }));
        const grCountBefore = await database_1.prisma.goodsReceipt.count();
        ...
        const grCountAfter = await database_1.prisma.goodsReceipt.count();
        assert(grCountAfter === grCountBefore, 'GoodsReceipt transaction was completely rolled back');
        const checkPoItem = await database_1.prisma.purchaseOrderItem.findUnique({ where: { id: item.id } });
...
//# sourceMappingURL=m11-test.js.map