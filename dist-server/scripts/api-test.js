...
const auth_1 = require("../apps/api/src/middleware/auth");
async function runApiTests() {
    ...
    app.get('/test/admin-only', {
        preHandler: [(0, auth_1.requireRole)('SUPER_ADMIN')],
        handler: async () => ({ status: 'admin_granted' }),
    });
    app.get('/test/product-write', {
        preHandler: [(0, auth_1.requirePermission)('product.create')],
    ...
    const extractCookie = (res, cookieName = 'autoparts_session') => {
    ...
    try {
        ...
        await test('Verify stored password uses genuine Argon2id (Gate A)', async () => {
            if (!registeredUserId)
                throw new Error('No registered user to inspect');
            const dbUser = await database_1.prisma.user.findUnique({ where: { id: registeredUserId } });
            if (!dbUser)
            ...
            const isValid = await argon2_1.default.verify(dbUser.passwordHash, testPassword);
            if (!isValid) {
        ...
        await test('POST /api/v1/auth/login succeeds with valid credentials', async () => {
            ...
            if (res.statusCode !== 200)
                throw new Error(`Expected 200, got ${res.statusCode}`);
            const body = JSON.parse(res.payload);
            if (body.data?.user?.email !== testRegEmail.toLowerCase())
                throw new Error('Login user mismatch');
            const loginCookie = extractCookie(res);
            if (!loginCookie)
                throw new Error('Session cookie not set on login');
        });
        await test('Verify raw session token is NEVER stored in database (Gate B)', async () => {
            if (!registeredCookie)
                throw new Error('No session cookie to inspect');
            const allSessions = await database_1.prisma.session.findMany({ where: { userId: registeredUserId } });
        ...
        await test('Reject login for deactivated user with 401 Account Deactivated', async () => {
            ...
            await database_1.prisma.user.update({ where: { id: registeredUserId }, data: { isActive: false } });
            const res = await app.inject({
                method: 'POST',
                url: '/api/v1/auth/login',
                payload: {
            ...
            await database_1.prisma.user.update({ where: { id: registeredUserId }, data: { isActive: true } });
            if (res.statusCode !== 401)
                throw new Error(`Expected 401, got ${res.statusCode}`);
            const body = JSON.parse(res.payload);
        ...
        await test('GET /api/v1/auth/me returns profile for authenticated user', async () => {
            ...
            const body = JSON.parse(res.payload);
            if (body.data?.user?.email !== testRegEmail.toLowerCase())
                throw new Error('Me profile email mismatch');
        ...
        await test('POST /api/v1/auth/change-password succeeds and updates hash with Argon2id', async () => {
            ...
            const dbUser = await database_1.prisma.user.findUnique({ where: { id: registeredUserId } });
            const verifyNew = await argon2_1.default.verify(dbUser.passwordHash, newPassword);
            if (!verifyNew)
                throw new Error('New password failed Argon2 verification');
        });
        // --------------------------------------------------------------------------
        ...
        await test('Authenticated CUSTOMER accessing admin route returns 403 Forbidden', async () => {
            ...
            if (body.error?.code !== 'INSUFFICIENT_ROLE')
                throw new Error(`Expected INSUFFICIENT_ROLE, got ${body.error?.code}`);
        });
        await test('Authenticated CUSTOMER accessing protected permission route returns 403 Forbidden', async () => {
            ...
            if (body.error?.code !== 'INSUFFICIENT_PERMISSIONS')
                throw new Error(`Expected INSUFFICIENT_PERMISSIONS, got ${body.error?.code}`);
        });
        await test('Authenticated SUPER_ADMIN accessing admin route returns 200 OK', async () => {
            ...
            if (res.statusCode !== 200)
                throw new Error(`Expected 200 OK for admin, got ${res.statusCode}`);
        });
        ...
        await test('Audit trail records authentication events in database', async () => {
            ...
            if (!actions.includes('REGISTERED'))
                throw new Error('REGISTERED audit log missing');
            if (!actions.includes('LOGIN_SUCCESS'))
                throw new Error('LOGIN_SUCCESS audit log missing');
            if (!actions.includes('LOGOUT'))
                throw new Error('LOGOUT audit log missing');
            if (!actions.includes('PASSWORD_CHANGED'))
                throw new Error('PASSWORD_CHANGED audit log missing');
        ...
        // Clean up test user
        await database_1.prisma.user.delete({ where: { id: registeredUserId } });
        console.log(`\n======================================================`);
