...
const fitment_service_1 = require("../apps/api/src/services/fitment.service");
...
async function runM4TestSuite() {
    ...
    const passwordHash = await argon2_1.default.hash('Test@123456', {
    ...
    // 1. Super Admin User
    const superAdmin = await database_1.prisma.user.upsert({
    ...
    const superAdminRole = await database_1.prisma.role.findUniqueOrThrow({ where: { name: 'SUPER_ADMIN' } });
    ...
    const customerUser = await database_1.prisma.user.upsert({
    ...
    await database_1.prisma.productFitment.deleteMany({
        where: { product: { sku: { in: ['TEST-PAD-M4-A', 'TEST-PAD-M4-B', 'MISLEADING-PAD-YARIS', 'OEM-SIMILARITY-PART'] } } },
    });
    await database_1.prisma.productPrice.deleteMany({
        where: { product: { sku: { in: ['TEST-PAD-M4-A', 'TEST-PAD-M4-B', 'MISLEADING-PAD-YARIS', 'OEM-SIMILARITY-PART'] } } },
    });
    await database_1.prisma.productCrossReference.deleteMany({
        where: { product: { sku: { in: ['TEST-PAD-M4-A', 'TEST-PAD-M4-B', 'MISLEADING-PAD-YARIS', 'OEM-SIMILARITY-PART'] } } },
    });
    ...
    const staleMake = await database_1.prisma.vehicleMake.findUnique({ where: { slug: 'mitsubishi-test' } });
    if (staleMake) {
        const models = await database_1.prisma.vehicleModel.findMany({ where: { makeId: staleMake.id } });
        const modelIds = models.map((m) => m.id);
        const gens = await database_1.prisma.vehicleGeneration.findMany({ where: { modelId: { in: modelIds } } });
        const genIds = gens.map((g) => g.id);
        const variants = await database_1.prisma.vehicleVariant.findMany({ where: { generationId: { in: genIds } } });
        const variantIds = variants.map((v) => v.id);
        await database_1.prisma.productFitment.deleteMany({ where: { vehicleVariantId: { in: variantIds } } });
        await database_1.prisma.vehicleVariant.deleteMany({ where: { id: { in: variantIds } } });
        await database_1.prisma.vehicleEngine.deleteMany({ where: { engineCode: '4N15-MIVEC' } });
        await database_1.prisma.vehicleGeneration.deleteMany({ where: { id: { in: genIds } } });
        await database_1.prisma.vehicleModel.deleteMany({ where: { id: { in: modelIds } } });
        await database_1.prisma.vehicleMake.delete({ where: { id: staleMake.id } });
    }
    await database_1.prisma.vehicleEngine.deleteMany({ where: { engineCode: '4N15-MIVEC' } });
    ...
    await recordTest('GET /api/v1/vehicles/makes returns active makes publicly without authentication', async () => {
        ...
        assert(json.makes.some((m) => m.id === testMakeId), 'Created make not found in list');
    });
    ...
    await recordTest('GET /api/v1/vehicles/generations?modelId=... returns generations for model', async () => {
        ...
        assert(res.statusCode === 200, `Expected 200, got ${res.statusCode}`);
        const json = JSON.parse(res.body);
        assert(json.generations.some((g) => g.id === testGenId), 'Generation not found');
    });
    await recordTest('GET /api/v1/vehicles/variants?generationId=... returns variants with full hierarchy', async () => {
        ...
        const vA = json.variants.find((v) => v.id === testVariantIdA);
    ...
    const category = await database_1.prisma.category.findFirstOrThrow({ where: { slug: 'brakes' } });
    const brand = await database_1.prisma.brand.findFirstOrThrow({ where: { slug: 'akebono' } });
    ...
    await recordTest('NEGATIVE TEST 1: Product name contains car model but lacks ProductFitment -> NOT COMPATIBLE', async () => {
        const misleadingProduct = await database_1.prisma.product.create({
            data: {
        ...
        await database_1.prisma.product.delete({ where: { id: misleadingProduct.id } });
    });
    ...
    await recordTest('NEGATIVE TEST 2: OEM cross-reference match without explicit ProductFitment -> NOT COMPATIBLE', async () => {
        const oemMatchProduct = await database_1.prisma.product.create({
            data: {
        ...
        assert(json.reason === 'NO_FITMENT_RECORD', 'Expected NO_FITMENT_RECORD');
        await database_1.prisma.product.delete({ where: { id: oemMatchProduct.id } });
    });
    ...
    await recordTest('NEGATIVE TEST 4: Missing vehicle specification returns INSUFFICIENT_VEHICLE_SPECIFICATION', async () => {
        const result = await fitment_service_1.FitmentService.checkProductFitment(testProductIdA, null);
    ...
    await recordTest('GET /api/v1/products?vehicleVariantId=... returns only compatible products', async () => {
        ...
        assert(json.data.some((p) => p.id === testProductIdA), 'Product A should be included (has fitment)');
        assert(!json.data.some((p) => p.id === testProductIdB), 'Product B MUST NOT be included (no fitment for variant A)');
    });
    ...
    await recordTest('GET /api/v1/vehicles/variants/:variantId/products returns compatible products list', async () => {
        ...
        assert(json.items.some((p) => p.id === testProductIdA), 'Product A missing from variant compatible products');
    });
    // ----------------------------------------------------------------------------
    ...
    await recordTest('Teardown: Safely cleanup test fitments, products, and vehicle hierarchy', async () => {
        if (testFitmentId) {
            await database_1.prisma.productFitment.deleteMany({ where: { productId: { in: [testProductIdA, testProductIdB] } } });
        }
        if (testProductIdA || testProductIdB) {
            await database_1.prisma.productPrice.deleteMany({ where: { productId: { in: [testProductIdA, testProductIdB] } } });
            await database_1.prisma.product.deleteMany({ where: { id: { in: [testProductIdA, testProductIdB] } } });
        }
        if (testVariantIdA || testVariantIdB) {
            await database_1.prisma.vehicleVariant.deleteMany({ where: { id: { in: [testVariantIdA, testVariantIdB] } } });
        }
        if (testEngineId) {
            await database_1.prisma.vehicleEngine.deleteMany({ where: { id: testEngineId } });
        }
        if (testGenId) {
            await database_1.prisma.vehicleGeneration.deleteMany({ where: { id: testGenId } });
        }
        if (testModelId) {
            await database_1.prisma.vehicleModel.deleteMany({ where: { id: testModelId } });
        }
        if (testMakeId) {
            await database_1.prisma.vehicleMake.deleteMany({ where: { id: testMakeId } });
        }
        await database_1.prisma.user.deleteMany({ where: { email: { in: ['m4-admin@mobex.co.th', 'm4-customer@test.com'] } } });
    });
    // ----------------------------------------------------------------------------
    // RESULTS SUMMARY
    // ----------------------------------------------------------------------------
...
//# sourceMappingURL=m4-test.js.map