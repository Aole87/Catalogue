...
const inventory_service_1 = require("../apps/api/src/services/inventory.service");
const results = [];
...
async function runM10TestSuite() {
    ...
    let salesUser = await database_1.prisma.user.findFirst({
        where: { roles: { some: { role: { name: 'SALES_REP' } } } },
    });
    if (!salesUser) {
        let salesRole = await database_1.prisma.role.findUnique({ where: { name: 'SALES_REP' } });
        if (!salesRole) {
            salesRole = await database_1.prisma.role.create({
                data: { name: 'SALES_REP', description: 'Sales Representative' },
            });
        }
        const passwordHash = adminUser.passwordHash;
        salesUser = await database_1.prisma.user.create({
            data: {
    ...
    let mainWarehouse = await database_1.prisma.warehouse.findFirst({
        where: { code: 'WH-MAIN', deletedAt: null },
    });
    if (!mainWarehouse) {
        mainWarehouse = await database_1.prisma.warehouse.create({
            data: {
    ...
    let secondaryWarehouse = await database_1.prisma.warehouse.findFirst({
        where: { code: 'WH-BRANCH-2', deletedAt: null },
    });
    if (!secondaryWarehouse) {
        secondaryWarehouse = await database_1.prisma.warehouse.create({
            data: {
    ...
    let testProduct = await database_1.prisma.product.findFirst({
    ...
    if (!testProduct) {
        ...
        testProduct = await database_1.prisma.product.create({
            data: {
    ...
    async function setupInventoryItem(warehouseId, productId, onHand, reserved = 0, reorderPoint = 10, safetyStock = 5) {
        const existing = await database_1.prisma.inventoryItem.findFirst({
            where: { warehouseId, productId },
        });
        if (existing) {
            return await database_1.prisma.inventoryItem.update({
                where: { id: existing.id },
                data: { onHand, reserved, reorderPoint, safetyStock },
            });
        }
        else {
            return await database_1.prisma.inventoryItem.create({
                data: {
                    warehouseId,
                    productId,
                    onHand,
    ...
    await recordTest('M10-T06: Concurrent Reservation Protection (Anti-Overselling Race Condition Test)', async () => {
        ...
        const requests = Array.from({ length: 5 }).map((_, i) => app.inject({
            method: 'POST',
            url: '/api/v1/inventory/reservations',
            headers: { cookie: adminCookie },
            payload: {
        ...
        const finalItem = await database_1.prisma.inventoryItem.findFirst({
    ...
    await recordTest('M10-T07: Reservation Release Restores Available Stock', async () => {
        ...
        let item = await database_1.prisma.inventoryItem.findFirst({
            where: { warehouseId: mainWarehouse.id, productId: testProduct.id },
        });
        ...
        item = await database_1.prisma.inventoryItem.findFirst({
            where: { warehouseId: mainWarehouse.id, productId: testProduct.id },
        });
    ...
    await recordTest('M10-T08: Reservation Commit Deducts onHand and Reserved Stock', async () => {
        ...
        const item = await database_1.prisma.inventoryItem.findFirst({
            where: { warehouseId: mainWarehouse.id, productId: testProduct.id },
        });
    ...
    await recordTest('M10-T13: Stock Movement Ledger Generation on Mutation', async () => {
        ...
        const match = movements.find((m) => m.notes?.includes(adjReason));
    ...
    await recordTest('M10-T14: Before and After Quantities Recorded Accurately', async () => {
        ...
        const movement = await database_1.prisma.stockMovement.findFirst({
    ...
    await recordTest('M10-T15: Warehouse Transfer Atomicity (Atomic Dual Mutation)', async () => {
        ...
        const sourceItem = await database_1.prisma.inventoryItem.findFirst({
            where: { warehouseId: mainWarehouse.id, productId: testProduct.id },
        });
        assert(sourceItem.onHand === 35, `Source onHand must be 35, got ${sourceItem.onHand}`);
        // Verify Branch 2 onHand incremented (10 + 15 = 25)
        const targetItem = await database_1.prisma.inventoryItem.findFirst({
    ...
    await recordTest('M10-T17: Low-Stock Calculation (available <= reorderPoint)', async () => {
        ...
        const body = JSON.parse(res.body);
        const item = body.data.find((i) => i.productId === testProduct.id);
    ...
    await recordTest('M10-T24: Shipping -> Inventory Boundary Preservation', async () => {
        ...
        const shipments = await database_1.prisma.shipment.findMany({ take: 1 });
    ...
    await recordTest('M10-T28: Stock Movement Ledger Append-Only Behavior', async () => {
        const totalMovementsBefore = await database_1.prisma.stockMovement.count();
        await setupInventoryItem(mainWarehouse.id, testProduct.id, 50, 0);
        await app.inject({
            method: 'POST',
            url: '/api/v1/inventory/adjustments',
        ...
        const totalMovementsAfter = await database_1.prisma.stockMovement.count();
    ...
    await recordTest('M10-T29: AuditLog Entry Creation for Inventory Actions', async () => {
        const initialLogs = await database_1.prisma.auditLog.count({
        ...
        const finalLogs = await database_1.prisma.auditLog.count({
            where: { action: { startsWith: 'INVENTORY_' } },
        });
    ...
    await recordTest('M10-R02: DAMAGED does not increase saleable stock', async () => {
        ...
        const damageMv = await database_1.prisma.stockMovement.findFirst({
    ...
    await recordTest('M10-R04: QUARANTINE stock cannot satisfy checkout availability', async () => {
        ...
        const data = JSON.parse(res.body).data;
        const whStock = data.warehouses.find((w) => w.warehouseId === mainWarehouse.id);
    ...
    await recordTest('M10-EXP01: Active reservation expiration via background sweep', async () => {
        ...
        const reservation = await database_1.prisma.stockReservation.create({
            data: {
                warehouseId: mainWarehouse.id,
                productId: testProduct.id,
                quantity: 5,
        ...
        // Run sweep
        const sweepResult = await inventory_service_1.InventoryService.expireStaleReservations(adminUser.id);
        ...
        const updated = await database_1.prisma.stockReservation.findUnique({
            where: { id: reservation.id },
        });
        assert(updated.status === database_1.ReservationStatus.EXPIRED, `Reservation status must be EXPIRED, got ${updated.status}`);
    ...
    await recordTest('M10-EXP02: Expired reservation cannot be committed', async () => {
        ...
        const expiredReservation = await database_1.prisma.stockReservation.findFirst({
            where: { status: database_1.ReservationStatus.EXPIRED, productId: testProduct.id },
        });
    ...
    await recordTest('M10-IDOR01: Customer A -> Customer B reservation isolation (403 Forbidden)', async () => {
        ...
        await setupInventoryItem(mainWarehouse.id, testProduct.id, 20, 0);
        const customerAOrder = await database_1.prisma.order.findFirst({
        ...
        const resv = await inventory_service_1.InventoryService.reserveStock({
...
//# sourceMappingURL=m10-test.js.map