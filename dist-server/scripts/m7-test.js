...
const promptpay_provider_1 = require("../apps/api/src/services/payment/providers/promptpay.provider");
const test_provider_1 = require("../apps/api/src/services/payment/providers/test.provider");
const payment_state_machine_1 = require("../apps/api/src/services/payment/payment-state-machine");
const results = [];
...
async function runM7TestSuite() {
    ...
    await recordTest('Payment Creation: Server is Sole Authority for Amount & Currency (THB)', async () => {
        ...
        assert(testOrder1.status === database_1.OrderStatus.PENDING_PAYMENT, 'Order starts in PENDING_PAYMENT');
        ...
        assert(payment.status === database_1.PaymentStatus.PENDING, 'Payment status must be PENDING');
        ...
        const orderInDb = await database_1.prisma.order.findUnique({ where: { id: testOrder1.id } });
        assert(orderInDb?.status === database_1.OrderStatus.PENDING_PAYMENT, 'Order in DB must remain PENDING_PAYMENT');
    });
    // ----------------------------------------------------------------------------
    ...
    await recordTest('State Machine: Allowed Legal Transitions (PENDING -> PAID -> REFUNDED)', async () => {
        payment_state_machine_1.PaymentStateMachine.validateTransition(database_1.PaymentStatus.PENDING, database_1.PaymentStatus.PAID);
        payment_state_machine_1.PaymentStateMachine.validateTransition(database_1.PaymentStatus.PAID, database_1.PaymentStatus.REFUNDED);
        payment_state_machine_1.PaymentStateMachine.validateTransition(database_1.PaymentStatus.PAID, database_1.PaymentStatus.PARTIALLY_REFUNDED);
        payment_state_machine_1.PaymentStateMachine.validateTransition(database_1.PaymentStatus.PARTIALLY_REFUNDED, database_1.PaymentStatus.REFUNDED);
        payment_state_machine_1.PaymentStateMachine.validateTransition(database_1.PaymentStatus.PENDING, database_1.PaymentStatus.FAILED);
        payment_state_machine_1.PaymentStateMachine.validateTransition(database_1.PaymentStatus.PENDING, database_1.PaymentStatus.CANCELLED);
    });
    await recordTest('State Machine: Rejection of Illegal Transitions (FAILED -> PAID, REFUNDED -> PAID, PENDING -> REFUNDED)', async () => {
        ...
        try {
            payment_state_machine_1.PaymentStateMachine.validateTransition(database_1.PaymentStatus.FAILED, database_1.PaymentStatus.PAID);
        }
        catch {
        ...
        try {
            payment_state_machine_1.PaymentStateMachine.validateTransition(database_1.PaymentStatus.REFUNDED, database_1.PaymentStatus.PAID);
        }
        catch {
        ...
        try {
            payment_state_machine_1.PaymentStateMachine.validateTransition(database_1.PaymentStatus.PENDING, database_1.PaymentStatus.REFUNDED);
        }
        catch {
    ...
    await recordTest('Webhook: Rejection of Invalid or Forged HMAC Signature (400 Bad Request)', async () => {
        ...
        const orderCheck = await database_1.prisma.order.findUnique({ where: { id: testOrder2.id } });
        assert(orderCheck?.status === database_1.OrderStatus.PENDING_PAYMENT, 'Order must remain PENDING_PAYMENT after forged webhook');
    });
    await recordTest('Webhook: Rejection of Expired Timestamp (Replay Attack Protection)', async () => {
        ...
        const { signature, timestamp } = test_provider_1.TestPaymentProvider.signPayload(replayPayload, expiredTimestamp);
        const webhookRes = await app.inject({
            method: 'POST',
            url: '/api/v1/payments/webhooks/TEST',
            headers: {
    ...
    await recordTest('Webhook: Authoritative Settlement with Valid Signature Transitions Order to PAYMENT_CONFIRMED', async () => {
        ...
        const { signature, timestamp } = test_provider_1.TestPaymentProvider.signPayload(validPayload);
        const webhookRes = await app.inject({
            method: 'POST',
            url: '/api/v1/payments/webhooks/TEST',
            headers: {
        ...
        // Verify DB states
        const paymentInDb = await database_1.prisma.payment.findUnique({ where: { id: testPayment2.id } });
        assert(paymentInDb?.status === database_1.PaymentStatus.PAID, 'Payment in DB must be PAID');
        assert(paymentInDb?.paidAt != null, 'paidAt timestamp must be populated');
        const orderInDb = await database_1.prisma.order.findUnique({ where: { id: testOrder2.id } });
        assert(orderInDb?.status === database_1.OrderStatus.PAYMENT_CONFIRMED, 'Order in DB must be PAYMENT_CONFIRMED');
        // Verify OrderStatusHistory entry was logged
        const statusHistory = await database_1.prisma.orderStatusHistory.findMany({ where: { orderId: testOrder2.id } });
        const paymentConfirmedEntry = statusHistory.find((h) => h.toStatus === database_1.OrderStatus.PAYMENT_CONFIRMED);
    ...
    await recordTest('Webhook Idempotency: Duplicate Event is Ignored with Zero Duplicate Side-Effects', async () => {
        ...
        const { signature, timestamp } = test_provider_1.TestPaymentProvider.signPayload(payload);
        ...
        const initialHistoryCount = await database_1.prisma.orderStatusHistory.count({ where: { orderId: testOrder3.id } });
        ...
        const postHistoryCount = await database_1.prisma.orderStatusHistory.count({ where: { orderId: testOrder3.id } });
    ...
    await recordTest('Financial Invariant: Webhook Underpayment is Strictly Rejected', async () => {
        ...
        const { signature, timestamp } = test_provider_1.TestPaymentProvider.signPayload(underpaidPayload);
        const res = await app.inject({
            method: 'POST',
            url: '/api/v1/payments/webhooks/TEST',
            headers: { 'x-signature': signature, 'x-timestamp': timestamp },
        ...
        const orderCheck = await database_1.prisma.order.findUnique({ where: { id: testOrder4.id } });
        assert(orderCheck?.status === database_1.OrderStatus.PENDING_PAYMENT, 'Order must remain PENDING_PAYMENT on underpayment');
    });
    await recordTest('Financial Invariant: Webhook Wrong Currency (USD) is Strictly Rejected', async () => {
        ...
        const { signature, timestamp } = test_provider_1.TestPaymentProvider.signPayload(usdPayload);
        const res = await app.inject({
            method: 'POST',
            url: '/api/v1/payments/webhooks/TEST',
            headers: { 'x-signature': signature, 'x-timestamp': timestamp },
    ...
    await recordTest('PromptPay: Generates Valid EMVCo Payload with CRC-16 Checksum', async () => {
        const promptpayProvider = new promptpay_provider_1.PromptPayProvider();
    ...
    await recordTest('Bank Transfer: Customer Submits Slip -> Status is PENDING_REVIEW', async () => {
        ...
        const paymentCheck = await database_1.prisma.payment.findUnique({ where: { id: testPaymentBank.id } });
        assert(paymentCheck?.status === database_1.PaymentStatus.PENDING, 'Payment must remain PENDING while slip is under review');
        const orderCheck = await database_1.prisma.order.findUnique({ where: { id: testOrderBank.id } });
        assert(orderCheck?.status === database_1.OrderStatus.PENDING_PAYMENT, 'Order must remain PENDING_PAYMENT while slip is under review');
    });
    ...
    await recordTest('Bank Transfer: Staff Verifies Slip -> Payment PAID & Order PAYMENT_CONFIRMED', async () => {
        ...
        // Verify DB
        const slipInDb = await database_1.prisma.paymentSlip.findUnique({ where: { id: submittedSlip.id } });
        ...
        const paymentInDb = await database_1.prisma.payment.findUnique({ where: { id: testPaymentBank.id } });
        assert(paymentInDb?.status === database_1.PaymentStatus.PAID, 'Payment must be transitioned to PAID');
        const orderInDb = await database_1.prisma.order.findUnique({ where: { id: testOrderBank.id } });
        assert(orderInDb?.status === database_1.OrderStatus.PAYMENT_CONFIRMED, 'Order must be transitioned to PAYMENT_CONFIRMED');
    });
    // ----------------------------------------------------------------------------
    ...
    await recordTest('Refund: Partial & Full Refund Transitions Payment and Order Correctly', async () => {
        ...
        assert(partialBody.payment.status === database_1.PaymentStatus.PARTIALLY_REFUNDED, 'Payment status should be PARTIALLY_REFUNDED');
        ...
        assert(fullBody.payment.status === database_1.PaymentStatus.REFUNDED, 'Payment status should be REFUNDED');
        const orderInDb = await database_1.prisma.order.findUnique({ where: { id: testOrderBank.id } });
        assert(orderInDb?.status === database_1.OrderStatus.REFUNDED, 'Order status should be REFUNDED upon full payment refund');
    });
    // ----------------------------------------------------------------------------
    ...
    await recordTest('IDOR Protection: Customer A Cannot Read or Manage Customer B Payment', async () => {
        ...
        const paymentsForOrder1 = await database_1.prisma.payment.findMany({ where: { orderId: testOrder1.id } });
    ...
    await recordTest('Concurrency Safety: Simultaneous Webhook Settlements Settle Exactly Once', async () => {
        ...
        const requests = Array.from({ length: 5 }).map(() => app.inject({
            method: 'POST',
            url: '/api/v1/payments/webhooks/TEST',
            headers: { 'x-signature': signature, 'x-timestamp': timestamp },
            payload,
        ...
        const historyEntries = await database_1.prisma.orderStatusHistory.findMany({
            where: {
        ...
        const orderInDb = await database_1.prisma.order.findUnique({ where: { id: testOrderConcurrent.id } });
        assert(orderInDb?.status === database_1.OrderStatus.PAYMENT_CONFIRMED, 'Concurrent order must be PAYMENT_CONFIRMED');
        const paymentInDb = await database_1.prisma.payment.findUnique({ where: { id: paymentConcurrent.id } });
        assert(paymentInDb?.status === database_1.PaymentStatus.PAID, 'Concurrent payment must be PAID');
    });
    // ----------------------------------------------------------------------------
    ...
    await recordTest('Payment API: GET /api/v1/payments/:id Returns Authoritative Hydrated Model', async () => {
        ...
        assert(data.status === database_1.PaymentStatus.PAID, 'Status must reflect settled PAID state');
    ...
    await recordTest('Slip Workflow: Staff Slip Rejection Records REJECTED Status and Reason', async () => {
        ...
        const rejectedInDb = await database_1.prisma.paymentSlip.findUnique({ where: { id: slipObj.id } });
        ...
        // Payment and Order must remain PENDING / PENDING_PAYMENT
        const paymentCheck = await database_1.prisma.payment.findUnique({ where: { id: paymentToReject.id } });
        assert(paymentCheck?.status === database_1.PaymentStatus.PENDING, 'Payment must remain PENDING after slip rejection');
    });
    ...
    await recordTest('Audit Trail: All Financial Transitions Produce Immutable AuditLog and PaymentEvent Records', async () => {
        const paymentEvents = await database_1.prisma.paymentEvent.findMany({
...
//# sourceMappingURL=m7-test.js.map