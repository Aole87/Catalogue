...
const order_state_machine_1 = require("../apps/api/src/services/order/order-state-machine");
const results = [];
...
async function runM9TestSuite() {
    ...
    const sampleProducts = await database_1.prisma.product.findMany({
        where: { isActive: true, deletedAt: null, prices: { some: { tier: database_1.PriceTier.GARAGE } } },
    ...
    async function createPaidOrder(cookie, product) {
        ...
        if (existingPayment) {
            await database_1.prisma.payment.update({
                ...
                data: {
                    status: database_1.PaymentStatus.PAID,
                    paidAt: new Date(),
                    amount: total,
                },
            });
    ...
    async function createDeliveredOrder(cookie, product) {
        const order = await createPaidOrder(cookie, product);
        await database_1.prisma.order.update({
            where: { id: order.id },
            data: { status: database_1.OrderStatus.DELIVERED },
        });
    ...
    await recordTest('M9-T01: Order State Machine Legal Sequential Transitions', async () => {
        ...
        assert(order_state_machine_1.OrderStateMachine.canTransition(database_1.OrderStatus.PENDING_PAYMENT, database_1.OrderStatus.PAYMENT_CONFIRMED), 'PENDING_PAYMENT -> PAYMENT_CONFIRMED must be valid');
        assert(order_state_machine_1.OrderStateMachine.canTransition(database_1.OrderStatus.PAYMENT_CONFIRMED, database_1.OrderStatus.PROCESSING), 'PAYMENT_CONFIRMED -> PROCESSING must be valid');
        assert(order_state_machine_1.OrderStateMachine.canTransition(database_1.OrderStatus.PROCESSING, database_1.OrderStatus.READY_FOR_SHIPMENT), 'PROCESSING -> READY_FOR_SHIPMENT must be valid');
        assert(order_state_machine_1.OrderStateMachine.canTransition(database_1.OrderStatus.READY_FOR_SHIPMENT, database_1.OrderStatus.SHIPPED), 'READY_FOR_SHIPMENT -> SHIPPED must be valid');
        assert(order_state_machine_1.OrderStateMachine.canTransition(database_1.OrderStatus.SHIPPED, database_1.OrderStatus.DELIVERED), 'SHIPPED -> DELIVERED must be valid');
        assert(order_state_machine_1.OrderStateMachine.canTransition(database_1.OrderStatus.DELIVERED, database_1.OrderStatus.RETURN_REQUESTED), 'DELIVERED -> RETURN_REQUESTED must be valid');
        assert(order_state_machine_1.OrderStateMachine.canTransition(database_1.OrderStatus.RETURN_REQUESTED, database_1.OrderStatus.RETURNED), 'RETURN_REQUESTED -> RETURNED must be valid');
        // Test terminal state
        assert(order_state_machine_1.OrderStateMachine.isTerminal(database_1.OrderStatus.CANCELLED), 'CANCELLED is terminal');
        assert(order_state_machine_1.OrderStateMachine.isTerminal(database_1.OrderStatus.REFUNDED), 'REFUNDED is terminal');
    });
    await recordTest('M9-T02: Illegal State Jump Rejection (e.g. PENDING_PAYMENT -> DELIVERED)', async () => {
        assert(!order_state_machine_1.OrderStateMachine.canTransition(database_1.OrderStatus.PENDING_PAYMENT, database_1.OrderStatus.DELIVERED), 'PENDING_PAYMENT -> DELIVERED must be invalid');
        assert(!order_state_machine_1.OrderStateMachine.canTransition(database_1.OrderStatus.PENDING_PAYMENT, database_1.OrderStatus.SHIPPED), 'PENDING_PAYMENT -> SHIPPED must be invalid');
        assert(!order_state_machine_1.OrderStateMachine.canTransition(database_1.OrderStatus.CANCELLED, database_1.OrderStatus.PROCESSING), 'CANCELLED -> PROCESSING must be invalid');
        let threw = false;
        try {
            order_state_machine_1.OrderStateMachine.validateTransition(database_1.OrderStatus.PENDING_PAYMENT, database_1.OrderStatus.DELIVERED);
        }
        catch (e) {
            threw = true;
    ...
    await recordTest('M9-T05: Customer IDOR Protection (Tenant/Customer Isolation & Public Masking)', async () => {
        ...
        assert(idorRes.statusCode === 403 || idorRes.statusCode === 404, `Expected 403 or 404 for cross-customer access by ID, got ${idorRes.statusCode}`);
        ...
        assert(idorNumberRes.statusCode === 403, `Expected 403 Forbidden for cross-customer orderNumber query, got ${idorNumberRes.statusCode}`);
        ...
        assert(timelineRes.statusCode === 403 || timelineRes.statusCode === 404, `Expected 403/404 for IDOR timeline access, got ${timelineRes.statusCode}`);
    ...
    await recordTest('M9-T06: Order Timeline Synthesis from StatusHistory, PaymentEvent, ShippingEvent', async () => {
        ...
        await database_1.prisma.orderStatusHistory.create({
            data: {
                orderId: paidOrder.id,
                fromStatus: database_1.OrderStatus.PENDING_PAYMENT,
                toStatus: database_1.OrderStatus.PAYMENT_CONFIRMED,
    ...
    await recordTest('M9-T07: Customer Cancellation of PENDING_PAYMENT Order', async () => {
        ...
        assert(unpaidOrder.status === database_1.OrderStatus.PENDING_PAYMENT, 'Order is PENDING_PAYMENT');
        const res = await app.inject({
            method: 'POST',
        ...
        const body = JSON.parse(res.body);
        const order = body.data ?? body.order;
        assert(order.status === database_1.OrderStatus.CANCELLED, 'Order status must be CANCELLED');
        // Verify in DB
        const dbOrder = await database_1.prisma.order.findUnique({
            where: { id: unpaidOrder.id },
            include: { statusHistory: true },
        });
        assert(dbOrder?.status === database_1.OrderStatus.CANCELLED, 'DB order is CANCELLED');
        assert(Boolean(dbOrder?.statusHistory.some((h) => h.toStatus === database_1.OrderStatus.CANCELLED)), 'Status history recorded');
    });
    await recordTest('M9-T08: Customer Cancellation of PAYMENT_CONFIRMED Order (Refund Eligibility Note)', async () => {
        ...
        assert(payment?.status === database_1.PaymentStatus.PAID, 'Payment status remains PAID awaiting accounting refund processing');
        // Verify status history notes refund eligibility
        const history = await database_1.prisma.orderStatusHistory.findFirst({
            where: { orderId: paidOrder.id, toStatus: database_1.OrderStatus.CANCELLED },
        });
    ...
    await recordTest('M9-T09: Customer Cancellation Rejection once Shipment is SHIPPED / DELIVERED', async () => {
        ...
        assert(errMsg.includes('Cannot cancel') || errMsg.includes('cannot be cancelled'), 'Error message must indicate cancellation is disallowed after shipping/delivery');
    });
    await recordTest('M9-T10: Customer Return Request on DELIVERED Order', async () => {
        ...
        const body = JSON.parse(res.body);
        const order = body.data ?? body.order;
        assert(order.status === database_1.OrderStatus.RETURN_REQUESTED, 'Order status must transition to RETURN_REQUESTED');
        // Verify in DB
        const dbOrder = await database_1.prisma.order.findUnique({
            where: { id: deliveredOrder.id },
            include: { statusHistory: true },
        });
        assert(dbOrder?.status === database_1.OrderStatus.RETURN_REQUESTED, 'DB order status is RETURN_REQUESTED');
        assert(Boolean(dbOrder?.statusHistory.some((h) => h.toStatus === database_1.OrderStatus.RETURN_REQUESTED)), 'Return request recorded in history');
    });
    await recordTest('M9-T11: Return Request Rejection on Unfulfilled / Unpaid Orders', async () => {
        ...
        const body = JSON.parse(res.body);
        const errMsg = body.error?.message ?? body.message ?? JSON.stringify(body);
        assert(errMsg.includes('DELIVERED') || errMsg.includes('Cannot request return'), 'Error message must state order must be DELIVERED');
    });
    await recordTest('M9-T12: Staff Return Approval -> Transition to RETURNED', async () => {
        ...
        const body = JSON.parse(res.body);
        const order = body.data ?? body.order;
        assert(order.status === database_1.OrderStatus.RETURNED, 'Order status must be RETURNED');
        // Verify in DB
        const dbOrder = await database_1.prisma.order.findUnique({
            where: { id: deliveredOrder.id },
        });
        assert(dbOrder?.status === database_1.OrderStatus.RETURNED, 'DB order status is RETURNED');
    });
    await recordTest('M9-T13: Staff Return Rejection -> Transition Back to DELIVERED', async () => {
        ...
        const body = JSON.parse(res.body);
        const order = body.data ?? body.order;
        assert(order.status === database_1.OrderStatus.DELIVERED, 'Order status must revert to DELIVERED');
        // Verify in DB
        const dbOrder = await database_1.prisma.order.findUnique({
            where: { id: deliveredOrder.id },
        });
        assert(dbOrder?.status === database_1.OrderStatus.DELIVERED, 'DB order status is DELIVERED');
    });
    ...
    await recordTest('M9-T15: Staff Order Filtering by Status, Payment Status, Date Range', async () => {
        ...
        for (const ord of orders) {
            assert(ord.status === database_1.OrderStatus.PAYMENT_CONFIRMED, 'Filtered status matches');
        }
    });
    await recordTest('M9-T16: Staff Order Sorting Whitelist Validation', async () => {
        ...
        const amounts = orders.map((o) => Number(o.grandTotal ?? o.totalAmount));
    ...
    await recordTest('M9-T18: Staff Status Update with State Machine Validation & Audit Log', async () => {
        ...
        const illegalRes = await app.inject({
            ...
            headers: { cookie: adminCookie },
            payload: {
                toStatus: database_1.OrderStatus.DELIVERED,
        ...
        const res = await app.inject({
            ...
            headers: { cookie: adminCookie },
            payload: {
                toStatus: database_1.OrderStatus.PROCESSING,
        ...
        const body = JSON.parse(res.body);
        const order = body.data ?? body.order;
        assert(order.status === database_1.OrderStatus.PROCESSING, 'Order status updated to PROCESSING');
        ...
        const history = await database_1.prisma.orderStatusHistory.findFirst({
            where: { orderId: paidOrder.id, toStatus: database_1.OrderStatus.PROCESSING },
        });
    ...
    await recordTest('M9-T20: Immutable Order Snapshots Preserved Across Master Catalog Changes', async () => {
        ...
        const initialPrice = await database_1.prisma.productPrice.findFirst({
            where: { productId: product.id, tier: database_1.PriceTier.GARAGE },
        });
        ...
        await database_1.prisma.productPrice.updateMany({
            where: { productId: product.id, tier: database_1.PriceTier.GARAGE },
        ...
        assert(Number(orderDetail.items[0].unitPrice) === originalPrice, `Snapshot price must remain ${originalPrice}, got ${orderDetail.items[0].unitPrice}`);
    ...
    await recordTest('M9-T21: Payment <-> Order Synchronization Helper & Out-of-Order Regression Protection', async () => {
        const testOrder = await createTestOrder(garageCookie, sampleProducts[0]);
        assert(testOrder.status === database_1.OrderStatus.PENDING_PAYMENT, 'Order is PENDING_PAYMENT');
        // 1. Simulate payment PAID resolution
        const resolvedStatus = order_state_machine_1.OrderStateMachine.resolveOrderStatusFromPayment(database_1.PaymentStatus.PAID, database_1.OrderStatus.PENDING_PAYMENT);
        assert(resolvedStatus === database_1.OrderStatus.PAYMENT_CONFIRMED, 'Resolved status must be PAYMENT_CONFIRMED');
        // 2. Simulate payment FAILED resolution
        const failedStatus = order_state_machine_1.OrderStateMachine.resolveOrderStatusFromPayment(database_1.PaymentStatus.FAILED, database_1.OrderStatus.PENDING_PAYMENT);
        assert(failedStatus === database_1.OrderStatus.PENDING_PAYMENT, 'Failed payment does not change order status');
        // 3. Stale event protection: Late PAID event must NOT regress SHIPPED, DELIVERED, or REFUNDED orders
        assert(order_state_machine_1.OrderStateMachine.resolveOrderStatusFromPayment(database_1.PaymentStatus.PAID, database_1.OrderStatus.SHIPPED) === database_1.OrderStatus.SHIPPED, 'Late PAID event must not regress SHIPPED order');
        assert(order_state_machine_1.OrderStateMachine.resolveOrderStatusFromPayment(database_1.PaymentStatus.PAID, database_1.OrderStatus.DELIVERED) === database_1.OrderStatus.DELIVERED, 'Late PAID event must not regress DELIVERED order');
        assert(order_state_machine_1.OrderStateMachine.resolveOrderStatusFromPayment(database_1.PaymentStatus.PAID, database_1.OrderStatus.REFUNDED) === database_1.OrderStatus.REFUNDED, 'Late PAID event must not regress REFUNDED order');
    });
    await recordTest('M9-T22: Shipment <-> Order Synchronization Helper & Out-of-Order Regression Protection', async () => {
        const shippedStatus = order_state_machine_1.OrderStateMachine.resolveOrderStatusFromShipment(database_1.ShipmentStatus.IN_TRANSIT, database_1.OrderStatus.PROCESSING);
        assert(shippedStatus === database_1.OrderStatus.SHIPPED, 'Shipment IN_TRANSIT maps to Order SHIPPED');
        const deliveredStatus = order_state_machine_1.OrderStateMachine.resolveOrderStatusFromShipment(database_1.ShipmentStatus.DELIVERED, database_1.OrderStatus.SHIPPED);
        assert(deliveredStatus === database_1.OrderStatus.DELIVERED, 'Shipment DELIVERED maps to Order DELIVERED');
        // Stale shipment event protection: Late IN_TRANSIT event must NOT regress DELIVERED, RETURN_REQUESTED, or RETURNED orders
        assert(order_state_machine_1.OrderStateMachine.resolveOrderStatusFromShipment(database_1.ShipmentStatus.IN_TRANSIT, database_1.OrderStatus.DELIVERED) === database_1.OrderStatus.DELIVERED, 'Late IN_TRANSIT event must not regress DELIVERED order');
        assert(order_state_machine_1.OrderStateMachine.resolveOrderStatusFromShipment(database_1.ShipmentStatus.IN_TRANSIT, database_1.OrderStatus.RETURNED) === database_1.OrderStatus.RETURNED, 'Late IN_TRANSIT event must not regress RETURNED order');
    });
    await recordTest('M9-T23: Concurrency Handling (Simultaneous Cancellation Single Transition)', async () => {
        ...
        assert(dbOrder?.status === database_1.OrderStatus.CANCELLED, 'Order is CANCELLED');
    });
    await recordTest('M9-T24: RBAC Enforcement (Non-staff Forbidden on Admin Endpoints)', async () => {
        ...
        const resStatus = await app.inject({
            ...
            headers: { cookie: garageCookie },
            payload: { toStatus: database_1.OrderStatus.PROCESSING },
        });
    ...
    await recordTest('M9-T25: ZERO STOCK MUTATION INVARIANT (No Stock Reservation or Deduction in M9)', async () => {
        ...
        assert(initialQty === finalQty, `ZERO STOCK MUTATION INVARIANT VIOLATED: initial=${initialQty}, final=${finalQty}. Stock must not mutate in M9 (deferred to M10)!`);
        // Verify no StockMovement records created in M9
        const stockMovements = await database_1.prisma.stockMovement.findMany({
...
//# sourceMappingURL=m9-test.js.map