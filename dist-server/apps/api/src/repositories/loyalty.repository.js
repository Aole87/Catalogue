...
class LoyaltyRepository {
    ...
    static async getOrCreateAccount(customerId) {
        return database_1.prisma.loyaltyAccount.upsert({
            where: { customerId },
            create: {
                customerId,
                pointsBalance: 0,
    ...
    static async findAccountByCustomerId(customerId) {
        return database_1.prisma.loyaltyAccount.findUnique({
            where: { customerId },
            include: {
                customer: {
                    include: {
    ...
    static async listAccounts(params) {
        ...
        const [total, accounts] = await Promise.all([
            database_1.prisma.loyaltyAccount.count({ where }),
            database_1.prisma.loyaltyAccount.findMany({
                where,
                skip,
                take: limit,
    ...
    static async recordTransaction(customerId, data, clientTx) {
      ...
      `;
            ...
            if (!account) {
                throw new app_error_1.BadRequestException('Loyalty account could not be found or locked');
            }
            ...
            if (newBalance < 0) {
                throw new app_error_1.BadRequestException(`Insufficient loyalty points. Current balance: ${currentBalance}, requested deduction: ${Math.abs(data.points)}`);
            }
            ...
            if (data.points > 0 && data.transactionType === client_1.LoyaltyTransactionType.EARN) {
                lifetimeEarned += data.points;
            }
            else if (data.points < 0 && data.transactionType === client_1.LoyaltyTransactionType.REDEEM) {
    ...
    static async hasOrderEarnedPoints(orderId) {
        const existing = await database_1.prisma.loyaltyTransaction.findFirst({
            where: {
                orderId,
...
exports.LoyaltyRepository = LoyaltyRepository;
