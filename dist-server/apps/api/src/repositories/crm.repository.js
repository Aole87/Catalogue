...
class CrmRepository {
    ...
    static async findCustomerByUserId(userId) {
        return database_1.prisma.customerProfile.findUnique({
            where: { userId },
            include: {
                user: {
                    select: {
    ...
    static async queryCustomers(params) {
        ...
        const [total, customers] = await Promise.all([
            database_1.prisma.customerProfile.count({ where }),
            database_1.prisma.customerProfile.findMany({
                where,
                skip,
                take: limit,
                orderBy: { createdAt: 'desc' },
    ...
    static async deleteCustomer(id) {
        const customer = await database_1.prisma.customerProfile.findUnique({ where: { id } });
        if (!customer)
            return null;
        return database_1.prisma.$transaction(async (tx) => {
            const now = new Date();
    ...
    static async listTags() {
        return database_1.prisma.customerTag.findMany({
            orderBy: { name: 'asc' },
            include: {
                _count: {
    ...
    static async createTag(name, color = '#3B82F6', description) {
        return database_1.prisma.customerTag.create({
            data: { name: name.trim(), color, description },
        });
    }
    static async assignTag(customerId, tagId) {
        return database_1.prisma.customerTagAssignment.upsert({
            where: {
    ...
    static async removeTag(customerId, tagId) {
        return database_1.prisma.customerTagAssignment.deleteMany({
    ...
    static async listSegments() {
        return database_1.prisma.customerSegment.findMany({
    ...
    static async findSegmentById(id) {
        return database_1.prisma.customerSegment.findFirst({
    ...
    static async createSegment(data) {
        return database_1.prisma.customerSegment.create({
            data: {
                name: data.name.trim(),
    ...
    static async deleteSegment(id) {
        return database_1.prisma.customerSegment.update({
            where: { id },
