...
async function settingsRoutes(fastify) {
    ...
    fastify.put('/api/v1/settings', { bodyLimit: 50 * 1024 * 1024 }, async (req, reply) => {
        const body = (req.body || {});
        if (body.payment)
            inMemorySettings.payment = { ...inMemorySettings.payment, ...body.payment };
        if (body.shipping)
            inMemorySettings.shipping = { ...inMemorySettings.shipping, ...body.shipping };
        if (body.general)
            inMemorySettings.general = { ...inMemorySettings.general, ...body.general };
        if (body.branding)
            inMemorySettings.branding = { ...inMemorySettings.branding, ...body.branding };
        if (body.typography)
            inMemorySettings.typography = { ...inMemorySettings.typography, ...body.typography };
        if (body.banners)
            inMemorySettings.banners = body.banners;
        if (body.menus)
            inMemorySettings.menus = body.menus;
        if (body.navigation)
            inMemorySettings.navigation = { ...inMemorySettings.navigation, ...body.navigation };
        if (body.storeInfo)
            inMemorySettings.storeInfo = { ...inMemorySettings.storeInfo, ...body.storeInfo };
        if (body.policies)
            inMemorySettings.policies = { ...inMemorySettings.policies, ...body.policies };
        try {
            if (body.payment) {
