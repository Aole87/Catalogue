...
export class BankTransferProvider implements PaymentProvider {
  ...
  async createPayment(params: CreatePaymentProviderParams): Promise<CreatePaymentProviderResult> {
    const providerReference = `BT-${Date.now()}-${crypto.randomBytes(4).toString('hex').toUpperCase()}`;
  ...
  async refundPayment(params: RefundPaymentProviderParams): Promise<RefundPaymentProviderResult> {
    const refundReference = `REF-BT-${Date.now()}-${crypto.randomBytes(3).toString('hex').toUpperCase()}`;
    return {
      success: true,
      refundReference,
