...
export class TestShippingProvider implements ShippingProvider {
   ...
   */
  static signPayload(payload: any, timestamp?: number): { 'x-shipping-signature': string; 'x-signature': string; 'x-timestamp': string } {
  ...
  verifyWebhookSignature(headers: Record<string, any>, rawBody: any): boolean {
    const signature = (headers['x-signature'] || headers['x-shipping-signature'] || '') as string;
