...
import { ShippingProviderFactory } from './shipping/providers/provider.factory';
import { ShipmentStateMachine } from './shipping/shipment-state-machine';
...
export class ShippingService {
  ...
  private static async verifyOrderOwnership(order: any, userId?: string, userRoles: string[] = []) {
    ...
    throw new ForbiddenException('You do not have permission to view or manage this shipment');
  }
  ...
  static async createShipment(input: CreateShipmentInput) {
    ...
    if (!recipientName || !phone || !addressLine1 || !province || !postalCode) {
      ...
      const defaultAddress = customerAddresses.find((a: any) => a.isDefault) || customerAddresses[0];
   ...
   */
  static async updateShipmentStatus(input: UpdateShipmentStatusInput) {
   ...
   */
  static async assignTracking(input: AssignTrackingInput) {
    ...
    if (shipment.status === ShipmentStatus.DELIVERED || shipment.status === ShipmentStatus.CANCELLED) {
      throw new BadRequestException(`Cannot assign tracking to a shipment in status '${shipment.status}'`);
    }
   ...
   */
  static async handleTrackingWebhook(providerName: string, rawBody: any, headers: Record<string, any>) {
    ...
    if (shipment) {
      try {
        await ShippingRepository.updateShipmentStatus({
          ...
          toStatus: webhookResult.status as ShipmentStatus,
   ...
   * Cancels a shipment.
   */
  static async cancelShipment(shipmentId: string, reason: string, actorId?: string) {
    const shipment = await ShippingRepository.findById(shipmentId);
    if (!shipment) {
      throw new NotFoundException('Shipment not found');
    }
    ...
    if (
      ...
      throw new BadRequestException(`Cannot cancel shipment that has already been shipped or delivered`);
    }
   ...
   */
  static async getShipmentById(shipmentId: string, userId?: string, userRoles: string[] = []) {
    const shipment = await ShippingRepository.findById(shipmentId);
    if (!shipment) {
      throw new NotFoundException('Shipment not found');
    }
   ...
   */
  static async getShipmentsByOrderId(orderId: string, userId?: string, userRoles: string[] = []) {
    const order = await OrderRepository.findById(orderId);
    if (!order) {
      throw new NotFoundException('Order not found');
    }
  ...
  static async getShipmentTracking(trackingNumber: string) {
    ...
    if (!shipment) {
      throw new NotFoundException('Shipment not found for tracking number');
    }
  ...
  static async getAdminShipments(
    params: { status?: ShipmentStatus; carrier?: string; page?: number; limit?: number },
    userRoles: string[] = []
  ) {
    const isStaff = userRoles.some((r) =>
      ['SUPER_ADMIN', 'ADMIN', 'STORE_MANAGER', 'SALES_REP', 'INVENTORY_CLERK', 'WAREHOUSE', 'ACCOUNTANT'].includes(r)
    );
    if (!isStaff) {
      throw new ForbiddenException('Staff privileges required to access admin shipments');
    }
