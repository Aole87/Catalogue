...
export class PromotionRepository {
   ...
   * Promotions CRUD
   */
  static async listPromotions(params: PromotionQueryParams) {
  ...
  static async findPromotionById(id: string) {
  ...
  static async createPromotion(data: {
    ...
    categoryIds?: string[];
    brandIds?: string[];
    rules?: { ruleType: string; ruleValue?: any }[];
  }) {
  ...
  static async updatePromotion(
    ...
    return prisma.$transaction(async (tx) => {
      ...
      if (data.minimumOrderAmount !== undefined)
        updateData.minimumOrderAmount = new Prisma.Decimal(data.minimumOrderAmount);
   ...
   */
  static async listCoupons(params: CouponQueryParams) {
    const page = Math.max(1, params.page || 1);
    const limit = Math.min(100, Math.max(1, params.limit || 20));
    const skip = (page - 1) * limit;
  ...
  static async findCouponById(id: string) {
    return prisma.coupon.findFirst({
      where: { id, deletedAt: null },
      include: {
        promotion: {
  ...
  static async countCustomerCouponRedemptions(couponId: string, customerId: string) {
    return prisma.couponRedemption.count({
      where: { couponId, customerId },
    });
  }

  static async listPromotionRedemptions(promotionId: string, limit = 50, skip = 0) {
