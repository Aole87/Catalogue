...
export class InventoryRepository {
   ...
   */
  static async findInventoryItems(params: InventoryQueryParams = {}) {
   ...
   */
  static async findByProductAndWarehouse(productId: string, warehouseId: string, tx?: Prisma.TransactionClient) {
   ...
   */
  static async findWithLock(productId: string, warehouseId: string, tx: Prisma.TransactionClient) {
  ...
  static async updateQuantities(
    id: string,
    data: { onHand?: number; reserved?: number; locationId?: string | null },
    tx: Prisma.TransactionClient
  ) {
  ...
  static async findReservationById(id: string, tx?: Prisma.TransactionClient) {
    const client = tx || prisma;
  ...
  static async findActiveReservationByReference(
    referenceType: string,
    referenceId: string,
    productId: string,
    tx?: Prisma.TransactionClient
  ...
  static async findReservationsByOrderId(orderId: string, tx?: Prisma.TransactionClient) {
    const client = tx || prisma;
  ...
  static async findMovements(params: MovementQueryParams = {}) {
    const page = Math.max(1, params.page || 1);
    const limit = Math.min(100, Math.max(1, params.limit || 20));
    const skip = (page - 1) * limit;
