...
import { prisma, PriceTier, CustomerType } from '@car-parts/database';
import { FastifyInstance } from 'fastify';
import { PricingService } from '../apps/api/src/services/pricing.service';
...
async function runM6TestSuite() {
  ...
  const sampleVehicle = await prisma.vehicleVariant.findFirst({
  ...
  await recordTest('Pricing Engine: General Tier Price Calculation', async () => {
    const lineItem = PricingService.calculateLineItem(p1, PriceTier.GENERAL, 2);
    const expectedGeneralPrice = p1.prices.find((p) => p.tier === PriceTier.GENERAL)?.price || p1.prices[0].price;
  ...
  await recordTest('Pricing Engine: Garage / Shop Tier Resolution', async () => {
    const lineItemGarage = PricingService.calculateLineItem(p1, PriceTier.GARAGE, 3);
    const garagePrice = p1.prices.find((p) => p.tier === PriceTier.GARAGE)?.price || p1.prices[0].price;
  ...
  await recordTest('Free Shipping Boundary: 1,999.99 THB -> standard shipping applies (50.00 THB)', async () => {
    ...
    const totals = PricingService.calculateTotals(items, PriceTier.GENERAL, 50.00);
  ...
  await recordTest('IDOR Check: Guest B cannot update Guest A cart item', async () => {
    ...
    guestBToken = initB.headers['x-session-token'] as string;
  ...
  await recordTest('Auth Login: Authenticate test garage user', async () => {
    ...
    customerAuthCookie = Array.isArray(setCookie) ? setCookie[0] : (setCookie as string);
  ...
  await recordTest('Cart Merge Scenario: Guest (P1 x2, P2 x1) + User (P1 x1, P3 x3)', async () => {
    ...
    const gToken = gRes.headers['x-session-token'] as string;
    await app.inject({
      method: 'POST',
      url: '/api/v1/cart/items',
    ...
    const itemP1 = mergedCart.items.find((i: any) => i.productId === p1.id);
    const itemP2 = mergedCart.items.find((i: any) => i.productId === p2.id);
    const itemP3 = mergedCart.items.find((i: any) => i.productId === p3.id);
  ...
  await recordTest('Security: Malicious client prices injected into checkout payload are discarded', async () => {
    ...
    const expectedItemPrice = Number(p1.prices.find((p) => p.tier === PriceTier.GENERAL)?.price || p1.prices[0].price);
  ...
  await recordTest('Concurrency: Simultaneous checkouts execute cleanly without cross-corruption', async () => {
    ...
    const g1Token = g1Res.headers['x-session-token'] as string;
    await app.inject({
      method: 'POST',
      url: '/api/v1/cart/items',
    ...
    const g2Token = g2Res.headers['x-session-token'] as string;
    await app.inject({
      method: 'POST',
      url: '/api/v1/cart/items',
  ...
  await recordTest('Snapshot Integrity: OrderItem retains original price/metadata after product master mutation', async () => {
    ...
    const p1Price = await prisma.productPrice.findFirst({ where: { productId: p1.id, tier: PriceTier.GARAGE } });
    if (p1Price) {
  ...
  await recordTest('GET /api/v1/orders/my-orders: Retrieves authenticated customer orders', async () => {
    ...
    assert(json.data.some((o: any) => o.orderNumber === createdOrderNumber), 'Created order appears in customer history');
  });
