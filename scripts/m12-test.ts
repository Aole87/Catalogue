...
import { prisma, Prisma, OrderStatus, PaymentStatus } from '@car-parts/database';
...
async function runM12TestSuite() {
  ...
  const superAdmin = await prisma.user.findFirst({ where: { email: 'admin@mobex.co.th' } });
  ...
  if (!testCategory) {
    testCategory = await prisma.category.create({ data: { name: 'Brake System', slug: `brake-system-${Date.now()}` } });
  }
  let testBrand = await prisma.brand.findFirst();
  if (!testBrand) {
    testBrand = await prisma.brand.create({ data: { name: 'Brembo', slug: `brembo-${Date.now()}` } });
  }
  ...
  const testProduct = await prisma.product.create({
    data: {
      ...
      prices: {
        create: [
          { tier: 'GENERAL', price: new Prisma.Decimal('1000.00'), currency: 'THB', isActive: true },
          { tier: 'GARAGE', price: new Prisma.Decimal('1000.00'), currency: 'THB', isActive: true },
        ],
      },
      inventoryItems: {
        create: {
  ...
  const testProduct2 = await prisma.product.create({
    data: {
      ...
      prices: {
        create: [
          { tier: 'GENERAL', price: new Prisma.Decimal('200.00'), currency: 'THB', isActive: true },
          { tier: 'GARAGE', price: new Prisma.Decimal('200.00'), currency: 'THB', isActive: true },
        ],
      },
      inventoryItems: {
        create: {
  ...
  await recordTest('M12-T05: Customer Activity timeline logging', async () => {
    ...
    assert(json.data.some((a: any) => a.eventType === 'PROFILE_UPDATED'), 'Logged activity found');
  });
  ...
  await recordTest('M12-T24: Minimum spend requirement rejection at checkout', async () => {
    const minCoup = await prisma.coupon.create({
      data: {
        promotionId: minOrderPromotion.id,
  ...
  await recordTest('M12-T25: Per-customer usage limit enforcement', async () => {
    ...
    const oneTimeCoup = await prisma.coupon.create({
      data: {
  ...
  await recordTest('M12-T26: Concurrent checkout coupon redemption race (usageLimit = 1, exactly 1 wins)', async () => {
    const exclPromo = await prisma.promotion.create({
      data: {
    ...
    const exclusiveCoup = await prisma.coupon.create({
      data: {
    ...
    const updatedCoup = await prisma.coupon.findUnique({ where: { id: exclusiveCoup.id } });
  ...
  await recordTest('M12-T28: Promotion snapshot saved on order at checkout', async () => {
    const orderWithPromo = await prisma.order.findFirst({
  ...
  await recordTest('M12-T30: Loyalty points earned upon order payment confirmation (1 pt per 100 THB)', async () => {
    ...
    const initAcc = await prisma.loyaltyAccount.findUnique({ where: { customerId: customerAProfile.id } });
    ...
    const updatedAcc = await prisma.loyaltyAccount.findUnique({ where: { customerId: customerAProfile.id } });
    assert(
      (updatedAcc?.pointsBalance || 0) === initBalance + 20,
      `Expected balance ${initBalance + 20}, got ${updatedAcc?.pointsBalance}`
    );
  });

  await recordTest('M12-T31: Double-earning prevention (idempotent earning on same order)', async () => {
    const accBefore = await prisma.loyaltyAccount.findUnique({ where: { customerId: customerAProfile.id } });
    ...
    const accAfter = await prisma.loyaltyAccount.findUnique({ where: { customerId: customerAProfile.id } });
  ...
  await recordTest('M12-T32: Loyalty points redemption at checkout (10 pts = 1 THB)', async () => {
    ...
    const accB = await prisma.loyaltyAccount.findUnique({ where: { customerId: customerBProfile.id } });
  ...
  await recordTest('M12-T34: Concurrent points redemption (cannot double-spend or go negative)', async () => {
    ...
    const acc = await prisma.loyaltyAccount.findUnique({ where: { customerId: customerBProfile.id } });
  ...
  await recordTest('M12-T36: Order cancellation / refund loyalty points compensating reversal', async () => {
    ...
    const accAfter = await prisma.loyaltyAccount.findUnique({ where: { customerId: customerAProfile.id } });
    assert(
      (accAfter?.pointsBalance || 0) === balanceBefore - 20,
      `Expected balance deducted by 20 pts upon cancellation, was ${balanceBefore} -> now ${accAfter?.pointsBalance}`
    );
  });
  ...
  await recordTest('M12-T38: Combined coupon + loyalty discount in single checkout', async () => {
    ...
    const stackPromo = await prisma.promotion.create({
      data: {
    ...
    const stackCoup = await prisma.coupon.create({
      data: {
  ...
  await recordTest('M12-T46: Historical order immutability - modifying a promotion later does NOT alter existing orders', async () => {
    ...
    const reloadedOrder = await prisma.order.findUnique({ where: { id: pastOrder!.id } });
  ...
  await recordTest('M12-T47: M7 Payment boundary isolation - Payment intent amount matches server post-discount total', async () => {
    const discountedOrder = await prisma.order.findFirst({
    ...
    if (payRes.statusCode === 200) {
      ...
      assert(
        Number(payJson.amount) === Number(discountedOrder!.grandTotal),
        'Payment intent amount matches discounted order grand total'
      );
    }
  });
