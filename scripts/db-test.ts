import { PrismaClient, CustomerType, PriceTier, OrderStatus, PaymentStatus, ShipmentStatus, FitmentStatus } from '@prisma/client';
...
async function runTests() {
  ...
  await test('Reject duplicate Fitment entry (Composite Unique Constraint)', async () => {
    const fitment = await prisma.productFitment.findFirst();
    if (!fitment) throw new Error('No fitment found');
  ...
  await test('Verify Decimal precision for monetary amounts (Decimal(12,2))', async () => {
    ...
    if (!brand || !category) throw new Error('Brand or category missing');
