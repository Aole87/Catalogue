...
async function runM5TestSuite() {
  ...
  let testMake: any = null;
  let testModel: any = null;
  let testGen: any = null;
  let testEngine: any = null;
  ...
  await recordTest('Level 1: GET /api/v1/vehicles/makes returns active vehicle makes', async () => {
    ...
    testMake = json.makes.find((m: any) => m.slug === 'toyota') || json.makes[0];
  ...
  const productWithFitment = await prisma.productFitment.findFirst({
  ...
  await recordTest('Negative Fitment: Different vehicle variant returns compatible=false and NO_FITMENT_RECORD', async () => {
    ...
    const allVariants = await prisma.vehicleVariant.findMany({ take: 2 });
