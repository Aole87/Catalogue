...
import { prisma, OrderStatus, ShipmentStatus, PriceTier } from '@car-parts/database';
import { FastifyInstance } from 'fastify';
import { FlashExpressProvider } from '../apps/api/src/services/shipping/providers/flash-express.provider';
import { KerryExpressProvider } from '../apps/api/src/services/shipping/providers/kerry-express.provider';
import { TestShippingProvider } from '../apps/api/src/services/shipping/providers/test.provider';
import { ShipmentStateMachine } from '../apps/api/src/services/shipping/shipment-state-machine';
...
async function runM8TestSuite() {
  ...
  await recordTest('M8-T01: Shipping Method Selection & Active Carrier Rates', async () => {
    ...
    const standardMethod = data.find(
      (m: any) => m.code === 'STANDARD' || m.carrier === 'Standard Delivery' || m.code === 'FLASH' || m.code === 'KERRY'
    );
  ...
  await recordTest('M8-T02: Payment Boundary Enforcement - Reject Fulfillment of Unpaid Order', async () => {
    ...
    assert(unpaidOrder.status === OrderStatus.PENDING_PAYMENT, 'Order must be in PENDING_PAYMENT status');
    ...
    assert(
      body.error?.message?.includes('Cannot fulfill unpaid order') || body.message?.includes('Cannot fulfill unpaid order'),
      'Error message must indicate payment is required'
    );
  });
  ...
  await recordTest('M8-T03: Shipment Creation for PAYMENT_CONFIRMED Order with Unique SHP Number', async () => {
    ...
    assert(shipment.status === ShipmentStatus.PENDING, 'Initial status must be PENDING');
    ...
    assert(shipment.events[0].status === ShipmentStatus.PENDING, 'Initial event must be PENDING');
  });
  ...
  await recordTest('M8-T04: Immutable Delivery Address Snapshotting', async () => {
    ...
    const checkShipment = await prisma.shipment.findUnique({ where: { id: shipment.id } });
    assert(checkShipment?.addressLine1 === '111 หมู่ 1 ที่อยู่เดิม', 'Historical addressLine1 must remain unchanged');
    assert(
      (checkShipment?.addressSnapshot as any)?.addressLine1 === '111 หมู่ 1 ที่อยู่เดิม',
      'Address snapshot JSON must remain immutable'
    );
  });
  ...
  await recordTest('M8-T06: Initial ShippingEvent Append-Only Log Created', async () => {
    ...
    const events = await prisma.shippingEvent.findMany({ where: { shipmentId: shipment.id } });
    assert(events.length >= 1, 'Must have at least 1 event');
    assert(events[0].status === ShipmentStatus.PENDING, 'Event status must be PENDING');
  ...
  await recordTest('M8-T09: Flash Express Provider - Tracking Generation & Format', async () => {
    const flashProvider = new FlashExpressProvider();
  ...
  await recordTest('M8-T10: Kerry Express Provider - Tracking Generation & Format', async () => {
    const kerryProvider = new KerryExpressProvider();
  ...
  await recordTest('M8-T11: Test Shipping Provider - Deterministic Flow & Signature Generation', async () => {
    const testProvider = new TestShippingProvider();
  ...
  await recordTest('M8-T12: Legal State Transition - READY_TO_SHIP -> SHIPPED Synchronizes Order Status', async () => {
    ...
    await prisma.shipment.update({
      ...
      data: { status: ShipmentStatus.READY_TO_SHIP, trackingNumber: 'TH9998887776F' },
    });
    ...
    const orderCheck = await prisma.order.findUnique({ where: { id: paidOrder.id } });
    assert(orderCheck?.status === OrderStatus.SHIPPED, 'Parent order status must synchronize to SHIPPED');
  });
  ...
  await recordTest('M8-T14: Inbound Courier Webhook - Valid HMAC Signature Ingestion', async () => {
    ...
    assert(updatedShipment?.status === ShipmentStatus.IN_TRANSIT, 'Shipment status updated to IN_TRANSIT');
  ...
  await recordTest('M8-T17: Out-of-Order Tracking Webhook Protection - Stale Event Cannot Downgrade DELIVERED', async () => {
    ...
    const checkShipment = await prisma.shipment.findUnique({ where: { id: shipment.id } });
    assert(checkShipment?.status === ShipmentStatus.DELIVERED, 'Status must NOT be downgraded from DELIVERED');
  });
  ...
  await recordTest('M8-T18: Final Delivery Transition - OUT_FOR_DELIVERY -> DELIVERED Synchronizes Order', async () => {
    ...
    const parentOrder = await prisma.order.findUnique({ where: { id: paidOrder.id } });
    assert(parentOrder?.status === OrderStatus.DELIVERED, 'Parent order must be DELIVERED upon delivery');
  });
  ...
  await recordTest('M8-T23: Public Tracking Lookup & Privacy Masking', async () => {
    ...
    await prisma.shipment.create({
      data: {
        ...
        events: {
          create: [
            { status: ShipmentStatus.PENDING, description: 'Created' },
            { status: ShipmentStatus.IN_TRANSIT, description: 'In Transit Hub 01', location: 'Rama 2 Hub' },
          ],
        },
      },
    });
  ...
  await recordTest('M8-T25: Zero Stock Mutation Invariant - Verifies Inventory Levels Unaltered in M8', async () => {
    ...
    assert(
      initialQty === finalQty,
      `ZERO STOCK MUTATION INVARIANT VIOLATED: initial=${initialQty}, final=${finalQty}. Stock must not mutate in M8!`
    );
  });
