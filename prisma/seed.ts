import { PrismaClient, CustomerType, PriceTier, FuelType, FitmentStatus, ProductReferenceType, InventoryMovementType } from '@prisma/client';
...
async function main() {
  ...
  const devPasswordHash = await argon2.hash('Admin@123456', {
  ...
  await prisma.customerProfile.upsert({
    ...
    create: {
      userId: shopUser.id,
      customerType: CustomerType.SHOP,
  ...
  const getOrCreateEngine = async (data: { engineCode: string; name: string; displacementCc?: number; cylinders?: number; fuelType?: FuelType; aspiration?: string }) => {
  ...
  const engL15B = await getOrCreateEngine({
    engineCode: 'L15BG', name: '1.5 VTEC Turbo', displacementCc: 1498, cylinders: 4, fuelType: FuelType.PETROL, aspiration: 'Turbocharged'
  });
  const eng1GD = await getOrCreateEngine({
    engineCode: '1GD-FTV', name: '2.8 D-4D VN Turbo', displacementCc: 2755, cylinders: 4, fuelType: FuelType.DIESEL, aspiration: 'Turbocharged'
  });
  const eng2GD = await getOrCreateEngine({
    engineCode: '2GD-FTV', name: '2.4 D-4D VN Turbo', displacementCc: 2393, cylinders: 4, fuelType: FuelType.DIESEL, aspiration: 'Turbocharged'
  });
  const engSky13 = await getOrCreateEngine({
    engineCode: 'SkyActiv-G 1.3', name: '1.3 SkyActiv-G Petrol', displacementCc: 1299, cylinders: 4, fuelType: FuelType.PETROL, aspiration: 'Naturally Aspirated'
  });
  const eng4JJ3 = await getOrCreateEngine({
    engineCode: '4JJ3-TCX', name: '3.0 Ddi BluePower VGS Turbo', displacementCc: 2999, cylinders: 4, fuelType: FuelType.DIESEL, aspiration: 'Turbocharged'
  });
  const engA25A = await getOrCreateEngine({
    engineCode: 'A25A-FKS', name: '2.5 Dynamic Force', displacementCc: 2487, cylinders: 4, fuelType: FuelType.PETROL, aspiration: 'Naturally Aspirated'
  });
  const eng3NR = await getOrCreateEngine({
    engineCode: '3NR-FE', name: '1.2 Dual VVT-i 3NR-FE', displacementCc: 1197, cylinders: 4, fuelType: FuelType.PETROL, aspiration: 'Naturally Aspirated'
  });
  const eng2NR = await getOrCreateEngine({
    engineCode: '2NR-FE', name: '1.5 Dual VVT-i 2NR-FE', displacementCc: 1496, cylinders: 4, fuelType: FuelType.PETROL, aspiration: 'Naturally Aspirated'
  });
  ...
  const productsData = [
    {
      ...
      crossRefs: [
        { type: ProductReferenceType.OEM, ref: '45022-TBA-A01' },
        { type: ProductReferenceType.AFTERMARKET, ref: 'GDB3494' },
      ],
      attributes: [
    ...
    },
    {
      ...
      crossRefs: [
        { type: ProductReferenceType.OEM, ref: '04465-33471' },
        { type: ProductReferenceType.OEM, ref: '04465-0K360' },
        { type: ProductReferenceType.CROSS_REFERENCE, ref: 'DB1774' },
      ],
      attributes: [
    ...
    },
    {
      ...
      crossRefs: [
        { type: ProductReferenceType.OEM, ref: '90919-01210' },
        { type: ProductReferenceType.AFTERMARKET, ref: 'BKR6EIX-11' },
      ],
      attributes: [
    ...
    },
    {
      ...
      crossRefs: [
        { type: ProductReferenceType.OEM, ref: '8-98165-071-0' },
        { type: ProductReferenceType.OEM, ref: '8-98000-001-0' },
      ],
      attributes: [
    ...
    },
    {
      ...
      crossRefs: [
        { type: ProductReferenceType.SUPPLIER, ref: 'MOBIL-0W40-4L-TH' },
      ],
      attributes: [
  ...
  for (const p of productsData) {
    ...
    await prisma.productPrice.upsert({
      where: { productId_tier: { productId: product.id, tier: PriceTier.GENERAL } },
      update: {},
      create: { productId: product.id, tier: PriceTier.GENERAL, price: p.prices.general, currency: 'THB' },
    });
    await prisma.productPrice.upsert({
      where: { productId_tier: { productId: product.id, tier: PriceTier.SHOP } },
      update: {},
      create: { productId: product.id, tier: PriceTier.SHOP, price: p.prices.shop, currency: 'THB' },
    });
    await prisma.productPrice.upsert({
      where: { productId_tier: { productId: product.id, tier: PriceTier.GARAGE } },
      update: {},
      create: { productId: product.id, tier: PriceTier.GARAGE, price: p.prices.garage, currency: 'THB' },
    });
